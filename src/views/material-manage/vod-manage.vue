          ...
          <div class="wrapper">
            <div class="flex space-between ">
              <div>
                ...
                <span style="margin-left: 15px;">
                  <el-select multiple v-model="videoQueryParam.groupIds" @change="handleQueryVideo" placeholder="全部分组" clearable>
                    <el-option v-for="group in videoGroups" :key="group.id" :label="group.name" :value="group.id"></el-option>
                  </el-select>
                </span>
              </div>
              <div>
            ...
            <div>
              <div style="padding-top: 20px;">
                <el-table ref="videoTable" :data="videoPage.records" row-key="id">
                  <el-table-column type="selection"></el-table-column>
                  <el-table-column prop="title" label="标题"></el-table-column>
                   <!-- <el-table-column prop="coverUrl" label="封面">
                     <template slot-scope="{row}">
                       <el-image :src="row.coverUrl" :preview-src-list="videoPage.records.map(v => v.coverUrl).filter(v => v)"></el-image>
                     </template>
                   </el-table-column> -->
          ...
          <div class="wrapper">
            <div class="flex space-between ">
              <div>
                ...
                <span style="margin-left: 15px;">
                  <el-select multiple v-model="audioQueryParam.groupIds"  placeholder="全部分组"  @change="handleQueryAudio" clearable>
                    <el-option v-for="group in audioGroups" :key="group.id" :label="group.name" :value="group.id"></el-option>
                  </el-select>
                </span>
              </div>
              <div>
            ...
            <div>
              <div style="padding-top: 20px;">
                <el-table ref="videoTable" :data="audioPage.records" row-key="id">
                  <el-table-column type="selection"></el-table-column>
                  <el-table-column prop="title" label="标题"></el-table-column>
                  <el-table-column prop="name" label="名称">
                    <template slot-scope="{row}">
...
export default {
  data () {
    return {
      ...
      videoQueryParam: {
        page: 1, size: 10, type: 3, name: '', groupIds: null
      },
      videoGroups: [],
      audioPage: {
        records: [],
      ...
      audioQueryParam: {
        page: 1, size: 10, type: 2, name: '', groupIds: null
      },
      audioGroups: [],
      videoDataSelection: [],
  ...
  mounted () {
    this.handleInitGroups(3).then(res => {
      res = res.data || []
      this.videoGroups = res
    })
    this.handleInitGroups(2).then(res => {
      res = res.data || []
      this.audioGroups = res
    })
    this.handleInitVodClassify()
    this.handleQueryVideo()
  },
  methods: {
    handleInitGroups (type) {
      return this.$get(`/material/group/list/noAll/${type}`)
    },
    ...
    hanleSaveVideoInfo () {
      this.$refs.videoForm.validate(valid => {
        if (valid) {
          ...
          this.$post('/material-file/saveVod', param).then(res => {
            if (res.code === 0) {
    ...
    hanleSaveAudioInfo () {
      this.$refs.audioForm.validate(valid => {
        if (valid) {
          this.$post('/material-file/saveVod', this.audioForm).then(res => {
            if (res.code === 0) {
              this.$notify.success('保存成功')
    ...
    handleQueryVideo () {
      this.$get('/material-file/pageSearch', { params: this.videoQueryParam }).then(res => {
        this.videoPage = res.data || { records: [], total: 0 }
      })
    },
    handleQueryAudio () {
      this.$get('/material-file/pageSearch', { params: this.audioQueryParam }).then(res => {
    ...
    handlePlayVideo (id, mediaType) {
      this.$get(`/material-file/getVodPlayAuth/${id}`).then(res => {
        if (res.code === 0) {
          const data = res.data
